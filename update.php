...
foreach ($servers as $server){
    ...
    if (file_exists($savepath.$server['path_dop'])==false){
		mkdir_new($savepath,$server['path_dop']);}
	if (file_exists($server['www'].$server['path_dop'])==false){
		mkdir_new($server['www'],$server['path_dop']);}
	...
	foreach ($upd_ser2 as $upd_ser3){
		...
		//if ($server['user']==true &&  $server['all_in_one'] == false) $newpath1=$server['path_dop']."/";
		//else $server['newpath']="";
		//else $newpath1="";
		//echo "url=".$newpath1."\n";
